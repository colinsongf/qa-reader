...
def evaluate_(ref_dict, pred_dict):
    f1 = 0
    em = 0
    total_count = 0
    skip_count = 0

    for query_id, answers in ref_dict.items():
        total_count += 1
        ...
        prediction = pred_dict[query_id][0]
        f1 += calc_f1_score(answers, prediction)
        em += calc_em_score(answers, prediction)
...
if __name__ == '__main__':
    ...
    AVG = (EM + F1) * 0.5
    print('AVG: {:.3f} F1: {:.3f} EM: {:.3f} TOTAL: {} SKIP: {} FILE: {}'.format(
        AVG, F1, EM, TOTAL, SKIP, sys.argv[2]))
