...
class QANet(object):
    ...
    def _embed(self):
        ...
        ph_emb = tf.reshape(h_emb, [-1, self.max_p_len, ph_emb.shape[-1]])
        qh_emb = tf.reshape(
    ...
    def _match(self):
        with tf.variable_scope("context_to_query_attention_layer"):
            ...
            self.p2q = tf.matmul(S_, q)
            self.q2p = tf.matmul(tf.matmul(S_, S_T), self.p_encode)
            attention_outputs = [self.p_encode, self.p2q,
    ...
    def _compute_loss(self):
        ...
        losses = loss(self.logits1, self.y1)
        losses2 = loss(self.logits2, self.y2)
